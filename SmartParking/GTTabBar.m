...
- (id)initWithFrame:(CGRect)frame buttonImages:(NSArray *)imageArray
{
    ...
    if (self)
	{
		...
		UIButton *btn;
		CGFloat width = RECT_W / [imageArray count];
		...
		//	btn.showsTouchWhenHighlighted = YES;
            ...
            if (i == 2)
            {
                if (isRetina||iPhone5)
                {
                    btn.frame = CGRectMake(width * i-5,-10, width+10, frame.size.height+10);

                }else{
                    btn.frame = CGRectMake(width * i-5,-20, width+10, frame.size.height+20);
...
- (void)removeTabAtIndex:(NSInteger)index
{
    ...
    CGFloat width = RECT_W / [self.buttons count];
...
{
    // Re-index the buttons
    CGFloat width = RECT_W / ([self.buttons count] + 1);
