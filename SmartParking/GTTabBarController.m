...
- (id)initWithViewControllers:(NSArray *)vcs imageArray:(NSArray *)arr
{
	...
	if (self != nil)
	{
		...
		transitionView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, RECT_W, containerView.frame.size.height - kTabBarHeight)];
		transitionView.backgroundColor =  [UIColor groupTableViewBackgroundColor];
		tabBar = [[GTTabBar alloc] initWithFrame:CGRectMake(0, RECT_H-kTabBarHeight, RECT_W, kTabBarHeight) buttonImages:arr];
...
- (void)setTabBarTransparent:(BOOL)yesOrNo
{
	...
	{
		transitionView.frame = CGRectMake(0, 0, RECT_W, containerView.frame.size.height - kTabBarHeight);
	}
}
...
{
    if ([[[NSUserDefaults standardUserDefaults] objectForKey:@"DYC"] isEqualToString:@"YES"])
    {
        index = 2;
        [[NSUserDefaults standardUserDefaults] setObject:@"NO" forKey:@"DYC"];
